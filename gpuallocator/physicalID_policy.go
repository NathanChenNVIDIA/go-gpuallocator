...
// Allocate GPUs following a simple policy.
func (p *physicalIDPolicy) AllocateSNV(available []*Device, required []*Device, size int, partitionGroupPhysIds []int) []*Device {
        if size <= 0 {
                return []*Device{}
        }
        ...
        allocated := append([]*Device{}, required...)
        allocated = append(allocated, availableSet.PhysicalIDSortedSliceSNV(partitionGroupPhysIds)[:size-len(allocated)]...)
        return allocated
}
